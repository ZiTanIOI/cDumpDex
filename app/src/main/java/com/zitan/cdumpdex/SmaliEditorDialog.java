...
public class SmaliEditorDialog {
    ...
    private void onSaveClick() {
        ...
        try {
            File outputFile = new File(dexInjectPath, className.replace('.', '_') + ".smali");
            ToolClass.createFile(outputFile);
            FileWriter writer = new FileWriter(outputFile);
            writer.write(currentSmali);
            writer.flush();
            writer.close();
            originalSmali = currentSmali;
            isModified = false;
            updateTitle();
            Toast.makeText(context, "已保存到: " + outputFile.getAbsolutePath(), Toast.LENGTH_LONG).show();
        } catch (IOException e) {
